...
import { useShowedMeHooks } from '../../hooks/useShowedMeHooks';
import { sortGoalsByRecent } from '@/lib/goalDateLabel';

export default function ShowedMeHooks() {
  const {
    showedMeHooks,
    loading,
    error,
  ...
  return (
    <>
      ...
      <p className="mb-3 text-sm text-muted-foreground">
        Hook copies for Showed Me reels, each tied to a goal. Edits save
        ...
        <table className="data-table">
          <thead>
            <tr>
              ...
              <th className="w-56">Goal</th>
              <th>Hook</th>
              <th className="w-32" aria-label="Actions" />
            </tr>
          </thead>
          <tbody>
            {showedMeHooks.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-4 py-12 text-center text-muted-foreground">
                  No hooks yet. Use &ldquo;Add row&rdquo; below to create one.
                </td>
              </tr>
            ) : (
              showedMeHooks.map((row, index) => {
                  ...
                  <tr key={row.id}>
                    ...
                    <td>
                      <select
                        ...
                        value={row.goalId ?? ''}
                        onChange={(e) =>
                          updateShowedMeHook(row.id, {
                            goalId: e.target.value ? Number(e.target.value) : null,
                          })
                        }
                        aria-label={`Goal for hook ${index + 1}`}
                      >
                        <option value="">—</option>
                        {sortedGoals.map((g) => (
                    ...
                    <td>
                      <TableInput
                        ...
                        onChange={(e) =>
                          updateShowedMeHook(row.id, { text: e.target.value })
                        }
                    ...
                    <td>
                      <div className="flex items-center gap-1">
                        ...
                        />
                        <DeleteRowButton
                          onClick={() => deleteShowedMeHook(row.id)}
      ...
      <AddRowButton onClick={addShowedMeHook} />
    </>
  );
}
