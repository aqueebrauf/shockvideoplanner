...
async function handlePresignUpload(payload) {
  const { goalId, planId, assetType, iteration, contentType, fileName } = payload;
  if (!goalId || !planId || !assetType || !contentType) {
    return jsonResponse(400, { error: 'goalId, planId, assetType, and contentType are required.' });
  }
...
export default async (req) => {
  ...
  try {
    ...
    return jsonResponse(status >= 400 && status < 600 ? status : 500, {
      error: err?.message ?? 'Person video API request failed.',
    });
  }
};
